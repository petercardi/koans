...
class AboutRegularExpressions < Neo::Koan
  ...
  def test_character_classes_give_options_for_a_character
    ...
    assert_equal animals.slice(0..2), animals.select { |a| a[/[cbr]at/] }
  end

  def test_slash_d_is_a_shortcut_for_a_digit_character_class
    assert_equal '42', "the number is 42"[/[0123456789]+/]
    assert_equal (40 + 2).to_s, "the number is 42"[/\d+/]
  end

  def test_character_classes_can_include_ranges
    assert_equal '42', "the number is 42"[/[0-9]+/]
    # wat?
    # assert_equal '142', "1the number is 42"[/[0-9][0-9]+/]
  end
...
## also learning nothing . . .
    assert_equal 'variable_1', "variable_1 = 42"[/[a-zA-Z0-9_]+/]
    assert_equal nil, "variable_1 = 42"[/\[w\]+/]
  end
...
# and now brackets without regexp? DEATH
...
# what the fuck is going on with these $?
  ...
  def test_gsub_is_like_find_and_replace_all
    assert_equal 'one asfeiuneat-t', "one asfeiuneatwo-three".gsub(/(t\w*)/) { $1[0, 1] }
# excuse you? And what is all this find-and-replace-ing using as its replace value?
  end
end
